...
def max_q_value(q_table, state):
    ...
    return float(q_table[state][np.argmax(q_table[state])])
...
def q_learning_update(q_table, state, action, reward, next_state, done, alpha, gamma):
    ...
    q_table[state][action] += float(alpha * td_error(td_target(reward, gamma, q_table, next_state, done), q_table, state, action))
...
def interaction_step(env, q_table, state, epsilon, alpha, gamma, rng):
    ...
    new_val = q_learning_update(q_table, state, action, reward, next_state, done, alpha, gamma)
...
def run_training_episode(env, q_table, epsilon, alpha, gamma, rng, max_steps=200):
    ...
    for i in range(max_steps):
        state, r, done = interaction_step(env, q_table, state, epsilon, alpha, gamma, rng)
        reward += r
        if done:
            break
...
def train_q_learning(env, num_episodes, alpha=0.8, gamma=0.95, epsilon_start=1.0, epsilon_min=0.01, epsilon_decay=0.99, seed=0, max_steps=200):
    ...
    for i in range(num_episodes):
        episode_returns.append(run_training_episode(env, q_table, epsilon_start, alpha, gamma, rng, max_steps))
        eps = decay_epsilon(eps, epsilon_decay, epsilon_min)
...
def extract_greedy_policy(q_table):
    ...
    for i in range(len(q_table)):
        ret.append(greedy_action(q_table, i))
